 ...
 * Created on: 2018-04-01 22:01:06.961287 by root
    ...
    s1ap_message *message_p)
{
    ...
    asn_enc_rval_t er;
    s1ap_DownlinkNASTransportIEs = &message_p->msg.s1ap_DownlinkNASTransportIEs;

    cb("<S1ap-DownlinkNASTransportIEs-PDU>\n", 35, app_key);
    xer_encode_local(&asn_DEF_S1ap_Criticality, &message_p->criticality, cb, app_key, 1);
    xer_encode_local(&asn_DEF_S1ap_ProcedureCode, &message_p->procedureCode, cb, app_key, 1);
    cb("    <S1ap-DownlinkNASTransportIEs>\n", 35, app_key);
    xer_encode_local(&asn_DEF_S1ap_MME_UE_S1AP_ID, &s1ap_DownlinkNASTransportIEs->mme_ue_s1ap_id, cb, app_key, 2);
    xer_encode_local(&asn_DEF_S1ap_ENB_UE_S1AP_ID, &s1ap_DownlinkNASTransportIEs->eNB_UE_S1AP_ID, cb, app_key, 2);
    xer_encode_local(&asn_DEF_S1ap_NAS_PDU, &s1ap_DownlinkNASTransportIEs->nas_pdu, cb, app_key, 2);
    /* Optional field */
    if (s1ap_DownlinkNASTransportIEs->presenceMask & S1AP_DOWNLINKNASTRANSPORTIES_HANDOVERRESTRICTIONLIST_PRESENT)
        xer_encode_local(&asn_DEF_S1ap_HandoverRestrictionList, &s1ap_DownlinkNASTransportIEs->handoverRestrictionList, cb, app_key, 2);
    /* Optional field */
    if (s1ap_DownlinkNASTransportIEs->presenceMask & S1AP_DOWNLINKNASTRANSPORTIES_SUBSCRIBERPROFILEIDFORRFP_PRESENT)
        xer_encode_local(&asn_DEF_S1ap_SubscriberProfileIDforRFP, &s1ap_DownlinkNASTransportIEs->subscriberProfileIDforRFP, cb, app_key, 2);
    cb("    </S1ap-DownlinkNASTransportIEs>\n", 36, app_key);
    cb("</S1ap-DownlinkNASTransportIEs-PDU>\n", 36, app_key);
    _ASN_ENCODED_OK(er);
}
    ...
    s1ap_message *message_p)
{
    ...
    asn_enc_rval_t er;
    s1ap_UEContextReleaseCompleteIEs = &message_p->msg.s1ap_UEContextReleaseCompleteIEs;

    cb("<S1ap-UEContextReleaseCompleteIEs-PDU>\n", 39, app_key);
    xer_encode_local(&asn_DEF_S1ap_Criticality, &message_p->criticality, cb, app_key, 1);
    xer_encode_local(&asn_DEF_S1ap_ProcedureCode, &message_p->procedureCode, cb, app_key, 1);
    cb("    <S1ap-UEContextReleaseCompleteIEs>\n", 39, app_key);
    xer_encode_local(&asn_DEF_S1ap_MME_UE_S1AP_ID, &s1ap_UEContextReleaseCompleteIEs->mme_ue_s1ap_id, cb, app_key, 2);
    xer_encode_local(&asn_DEF_S1ap_ENB_UE_S1AP_ID, &s1ap_UEContextReleaseCompleteIEs->eNB_UE_S1AP_ID, cb, app_key, 2);
    /* Optional field */
    if (s1ap_UEContextReleaseCompleteIEs->presenceMask & S1AP_UECONTEXTRELEASECOMPLETEIES_CRITICALITYDIAGNOSTICS_PRESENT)
        xer_encode_local(&asn_DEF_S1ap_CriticalityDiagnostics, &s1ap_UEContextReleaseCompleteIEs->criticalityDiagnostics, cb, app_key, 2);
    cb("    </S1ap-UEContextReleaseCompleteIEs>\n", 40, app_key);
    cb("</S1ap-UEContextReleaseCompleteIEs-PDU>\n", 40, app_key);
    _ASN_ENCODED_OK(er);
}
    ...
    s1ap_message *message_p)
{
    ...
    if (s1ap_HandoverCommandIEs->presenceMask & S1AP_HANDOVERCOMMANDIES_E_RABSUBJECTTODATAFORWARDINGLIST_PRESENT)
        s1ap_xer_print_s1ap_e_rabdataforwardinglist(cb, app_key, &s1ap_HandoverCommandIEs->e_RABSubjecttoDataForwardingList);
    /* Optional field */
    ...
    s1ap_message *message_p)
{
    ...
    asn_enc_rval_t er;
    s1ap_UplinkNASTransportIEs = &message_p->msg.s1ap_UplinkNASTransportIEs;

    cb("<S1ap-UplinkNASTransportIEs-PDU>\n", 33, app_key);
    xer_encode_local(&asn_DEF_S1ap_Criticality, &message_p->criticality, cb, app_key, 1);
    xer_encode_local(&asn_DEF_S1ap_ProcedureCode, &message_p->procedureCode, cb, app_key, 1);
    cb("    <S1ap-UplinkNASTransportIEs>\n", 33, app_key);
    xer_encode_local(&asn_DEF_S1ap_MME_UE_S1AP_ID, &s1ap_UplinkNASTransportIEs->mme_ue_s1ap_id, cb, app_key, 2);
    xer_encode_local(&asn_DEF_S1ap_ENB_UE_S1AP_ID, &s1ap_UplinkNASTransportIEs->eNB_UE_S1AP_ID, cb, app_key, 2);
    xer_encode_local(&asn_DEF_S1ap_NAS_PDU, &s1ap_UplinkNASTransportIEs->nas_pdu, cb, app_key, 2);
    xer_encode_local(&asn_DEF_S1ap_EUTRAN_CGI, &s1ap_UplinkNASTransportIEs->eutran_cgi, cb, app_key, 2);
    xer_encode_local(&asn_DEF_S1ap_TAI, &s1ap_UplinkNASTransportIEs->tai, cb, app_key, 2);
    /* Optional field */
    if (s1ap_UplinkNASTransportIEs->presenceMask & S1AP_UPLINKNASTRANSPORTIES_GW_TRANSPORTLAYERADDRESS_PRESENT)
        xer_encode_local(&asn_DEF_S1ap_TransportLayerAddress, &s1ap_UplinkNASTransportIEs->gW_TransportLayerAddress, cb, app_key, 2);
    cb("    </S1ap-UplinkNASTransportIEs>\n", 34, app_key);
    cb("</S1ap-UplinkNASTransportIEs-PDU>\n", 34, app_key);
    _ASN_ENCODED_OK(er);
}
    ...
    s1ap_message *message_p)
{
    ...
    asn_enc_rval_t er;
    s1ap_UEContextReleaseCommandIEs = &message_p->msg.s1ap_UEContextReleaseCommandIEs;

    cb("<S1ap-UEContextReleaseCommandIEs-PDU>\n", 38, app_key);
    xer_encode_local(&asn_DEF_S1ap_Criticality, &message_p->criticality, cb, app_key, 1);
    xer_encode_local(&asn_DEF_S1ap_ProcedureCode, &message_p->procedureCode, cb, app_key, 1);
    cb("    <S1ap-UEContextReleaseCommandIEs>\n", 38, app_key);
    xer_encode_local(&asn_DEF_S1ap_UE_S1AP_IDs, &s1ap_UEContextReleaseCommandIEs->uE_S1AP_IDs, cb, app_key, 2);
    xer_encode_local(&asn_DEF_S1ap_Cause, &s1ap_UEContextReleaseCommandIEs->cause, cb, app_key, 2);
    cb("    </S1ap-UEContextReleaseCommandIEs>\n", 39, app_key);
    cb("</S1ap-UEContextReleaseCommandIEs-PDU>\n", 39, app_key);
    _ASN_ENCODED_OK(er);
}
    ...
    s1ap_message *message_p)
{
    ...
    if (s1ap_DownlinkS1cdma2000tunnelingIEs->presenceMask & S1AP_DOWNLINKS1CDMA2000TUNNELINGIES_E_RABSUBJECTTODATAFORWARDINGLIST_PRESENT)
        s1ap_xer_print_s1ap_e_rabdataforwardinglist(cb, app_key, &s1ap_DownlinkS1cdma2000tunnelingIEs->e_RABSubjecttoDataForwardingList);
    /* Optional field */
...
asn_enc_rval_t s1ap_xer_print_s1ap_e_rabinformationlist(
    ...
    int i;
    asn_enc_rval_t er;
    for (i = 0; i < s1ap_E_RABInformationListIEs->s1ap_E_RABInformationItem.count; i++) {
        er = xer_encode(&asn_DEF_S1ap_E_RABInformationListItem, s1ap_E_RABInformationListIEs->s1ap_E_RABInformationItem.array[i], XER_F_BASIC, cb, app_key);
    }
    _ASN_ENCODED_OK(er);
}
