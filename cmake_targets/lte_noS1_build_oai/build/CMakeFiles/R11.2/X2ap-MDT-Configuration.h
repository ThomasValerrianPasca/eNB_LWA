/*
 * Generated by asn1c-0.9.24 (http://lionet.info/asn1c)
 * From ASN.1 module "X2AP-IEs"
 * 	found in "/home/oai/Downloads/LWA/openair2/X2AP/MESSAGES/ASN1/R11.2/X2AP-IEs.asn"
 * 	`asn1c -gen-PER`
 */

#ifndef	_X2ap_MDT_Configuration_H_
#define	_X2ap_MDT_Configuration_H_


#include <asn_application.h>

/* Including external dependencies */
#include "X2ap-MDT-Activation.h"
#include "X2ap-AreaScopeOfMDT.h"
#include "X2ap-MeasurementsToActivate.h"
#include "X2ap-ReportingTriggerMDT.h"
#include <constr_SEQUENCE.h>

#ifdef __cplusplus
extern "C" {
#endif

/* Forward declarations */
struct X2ap_ThresholdEventA2;
struct X2ap_PeriodicReportingMDT;
struct X2ap_IE_Extensions;

/* X2ap-MDT-Configuration */
typedef struct X2ap_MDT_Configuration {
	X2ap_MDT_Activation_t	 mdt_Activation;
	X2ap_AreaScopeOfMDT_t	 areaScopeOfMDT;
	X2ap_MeasurementsToActivate_t	 measurementsToActivate;
	X2ap_ReportingTriggerMDT_t	 reportingTriggerMDT;
	struct X2ap_ThresholdEventA2	*thresholdeventA2	/* OPTIONAL */;
	struct X2ap_PeriodicReportingMDT	*periodicReportingMDT	/* OPTIONAL */;
	struct X2ap_IE_Extensions	*iE_Extensions	/* OPTIONAL */;
	/*
	 * This type is extensible,
	 * possible extensions are below.
	 */
	
	/* Context for parsing across buffer boundaries */
	asn_struct_ctx_t _asn_ctx;
} X2ap_MDT_Configuration_t;

/* Implementation */
extern asn_TYPE_descriptor_t asn_DEF_X2ap_MDT_Configuration;

#ifdef __cplusplus
}
#endif

/* Referred external types */
#include "X2ap-ThresholdEventA2.h"
#include "X2ap-PeriodicReportingMDT.h"
#include "X2ap-IE-Extensions.h"

#endif	/* _X2ap_MDT_Configuration_H_ */
#include <asn_internal.h>
